...
const main = async (config: Config) => {
  ...
  let editionCount = config.startIndex;
  ...
  for (
    ...
    while (editionCount - config.startIndex <= cumulativeSize) {
      const [
        renderableLayers,
        nonRenderableLayers,
      ] = mapSelectedElementsToLayers(config, layers, editionCount);
      ...
      if (dna && !dnaSet.has(dna) && renderableLayers.length) {
        ...
        console.log(`${editionCount - config.startIndex}/${cumulativeSize}`);
